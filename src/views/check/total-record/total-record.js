...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            this.$axios.get('/managerapi/custome/grade/table/list', { params }).then(res => {
                this.loading = false;
                if (res.result) {
        ...
        openEdit(scope) {
            ...
            if (scope.row) {
                ...
                this.time = [];
                this.time2 = ''
                this.time2 = scope.row.sellingEndTime
                this.time[0] = scope.row.startTime;
                this.time[1] = scope.row.endTime;
                this.startTime = scope.row.startTime;
                this.endTime = scope.row.endTime;
                this.id = scope.row.id;
                this.isAvailable = scope.row.isAvailable + '';
                this.goods.ficti = scope.row.goods.ficti;
                this.deliverFee = scope.row.deliverFee;
            }
            if (scope.operation == 5) {
                this.$router.push({ path: `/check/record-detail/${scope.id}` })
