...
export default {
  ...
  methods: {
    deleteMsg(scope) {
      ...
      this.$confirm('你确定要删除这条信息吗, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
    }).then(() => {
        this.$axios.delete('/managerapi/custome/notice/delete', {
            params: {
                id: scope.row.id
            }
        }).then((res) => {
    ...
    getCompanyShop() {
      this.$axios.get('/managerapi/custome/notice/company/shop/get', {}).then(res => {
        ...
        this.$axios.get('/managerapi/custome/notice/all/shop/get',{}).then(res => {
          res.datas.forEach(item => {
            this.acceptUser[1]['children'].push(item)
          })
        })
        this.$axios.get('/managerapi/custome/notice/all/supervisors/get',{}).then(res => {
          res.datas.forEach(el => {
            this.acceptUser[2]['children'].push(el)
          })
        })
